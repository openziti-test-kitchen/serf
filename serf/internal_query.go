...
func newSerfQueries(serf *Serf, logger logger2.Logger, outCh chan<- Event, shutdownCh <-chan struct{}) (chan<- Event, error) {
...
func (s *serfQueries) handleQuery(q *Query) {
	...
	case listKeysQuery:
		s.handleListKeys(q)
	default:
		s.logger.Warnf("Unhandled internal query '%s'", queryName)
	}
}
...
func (s *serfQueries) handleConflict(q *Query) {
	...
	s.logger.Debugf("Got conflict resolution query for '%s'", node)
	...
	if err != nil {
		s.logger.Errorf("Failed to encode conflict query response: %v", err)
		return
	}

	// Send our answer
	if err := q.Respond(buf); err != nil {
		s.logger.Errorf("Failed to respond to conflict query: %v", err)
	}
}
...
func (s *serfQueries) sendKeyResponse(q *Query, resp *nodeKeyResponse) {
	...
	case internalQueryName(listKeysQuery):
		...
		if err := q.respondWithMessageAndResponse(raw, qresp); err != nil {
			s.logger.Errorf("Failed to respond to key query: %v", err)
			return
		}
	default:
		buf, err := encodeMessage(messageKeyResponseType, resp)
		if err != nil {
			s.logger.Errorf("Failed to encode key response: %v", err)
			return
		}
		if err := q.Respond(buf); err != nil {
			s.logger.Errorf("Failed to respond to key query: %v", err)
...
func (s *serfQueries) handleInstallKey(q *Query) {
	...
	if err != nil {
		s.logger.Errorf("Failed to decode key request: %v", err)
		goto SEND
	}

	if !s.serf.EncryptionEnabled() {
		response.Message = "No keyring to modify (encryption not enabled)"
		s.logger.Errorf("No keyring to modify (encryption not enabled)")
		goto SEND
	}

	s.logger.Infof("Received install-key query")
	if err := keyring.AddKey(req.Key); err != nil {
		response.Message = err.Error()
		s.logger.Errorf("Failed to install key: %s", err)
		goto SEND
	}
	...
	if s.serf.config.KeyringFile != "" {
		if err := s.serf.writeKeyringFile(); err != nil {
			response.Message = err.Error()
			s.logger.Errorf("Failed to write keyring file: %s", err)
			goto SEND
		}
	}
...
func (s *serfQueries) handleUseKey(q *Query) {
	...
	s.logger.Infof("Received use-key query")
	if err := keyring.UseKey(req.Key); err != nil {
		response.Message = err.Error()
		s.logger.Errorf("Failed to change primary key: %s", err)
		goto SEND
	}
...
func (s *serfQueries) handleRemoveKey(q *Query) {
	...
	s.logger.Infof("Received remove-key query")
	if err := keyring.RemoveKey(req.Key); err != nil {
		response.Message = err.Error()
		s.logger.Errorf("Failed to remove key: %s", err)
		goto SEND
	}
...
func (s *serfQueries) handleListKeys(q *Query) {
	...
	if !s.serf.EncryptionEnabled() {
		...
		s.logger.Errorf("Keyring is empty (encryption not enabled)")
		goto SEND
	}

	s.logger.Infof("Received list-keys query")
